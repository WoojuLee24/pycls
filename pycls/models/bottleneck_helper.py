...
class NormalMaxBlurPoolTransform(Module):
    ...
    def __init__(self, w_in, w_out, stride, _params):
        ...
        if stride != 1:
            ...
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
            # self.max_blur = NormalBlurPool(w_out, w_out, stride=stride, groups=w_out)
            self.max_blur = NormalBlurPool2(w_out, w_out, stride=stride, groups=w_out)
        else:
            self.a = conv2d(w_in, w_out, 3, stride=stride)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ParamMaxBlurPoolTransform(Module):
    """Basic transformation: [3x3 conv, BN, Relu] x2."""

    def __init__(self, w_in, w_out, stride, _params):
        super(ParamMaxBlurPoolTransform, self).__init__()
        if stride != 1:
            self.a = conv2d(w_in, w_out, 3, stride=1)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ResParamMaxBlurPoolBlock(Module):
    """Residual basic block: x + f(x), f = basic transform."""
...
class ParamMaxBlurPoolGroupTransform(Module):
    """Basic transformation: [3x3 conv, BN, Relu] x2."""

    def __init__(self, w_in, w_out, stride, _params):
        super(ParamMaxBlurPoolGroupTransform, self).__init__()
        if stride != 1:
            self.a = conv2d(w_in, w_out, 3, stride=1)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ResParamMaxBlurPoolGroupBlock(Module):
    """Residual basic block: x + f(x), f = basic transform."""

    def __init__(self, w_in, w_out, stride, params):
        super(ResParamMaxBlurPoolGroupBlock, self).__init__()
        self.proj, self.bn = None, None
        if (w_in != w_out) or (stride != 1):
...
class ParamBmvcBlurPoolGroupTransform(Module):
    """Basic transformation: [3x3 conv, BN, Relu] x2."""

    def __init__(self, w_in, w_out, stride, _params):
        super(ParamBmvcBlurPoolGroupTransform, self).__init__()
        if stride != 1:
            self.a = conv2d(w_in, w_out, 3, stride=1)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
            self.max_blur = BMVCBlurPool3x3(w_out, w_out, stride=stride, groups=w_out)
        else:
            self.a = conv2d(w_in, w_out, 3, stride=stride)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ResParamBmvcBlurPoolGroupBlock(Module):
    """Residual basic block: x + f(x), f = basic transform."""

    def __init__(self, w_in, w_out, stride, params):
        super(ResParamBmvcBlurPoolGroupBlock, self).__init__()
        self.proj, self.bn = None, None
        if (w_in != w_out) or (stride != 1):
            self.proj_blur = BMVCBlurPool3x3(w_in, w_in, stride=stride, groups=w_in)
            self.proj = conv2d(w_in, w_out, 1, stride=1)
            self.bn = norm2d(w_out)
        self.f = ParamBmvcBlurPoolGroupTransform(w_in, w_out, stride, params)
        self.af = activation()
...
class SortMaxBlurPoolTransform(Module):
    """Basic transformation: [3x3 conv, BN, Relu] x2."""

    def __init__(self, w_in, w_out, stride, _params):
        super(SortMaxBlurPoolTransform, self).__init__()
        if stride != 1:
            self.a = conv2d(w_in, w_out, 3, stride=1)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
            self.max_blur = SortBlurPool(w_out, w_out, stride=stride)
        else:
            self.a = conv2d(w_in, w_out, 3, stride=stride)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ResSortMaxBlurPoolBlock(Module):
    """Residual basic block: x + f(x), f = basic transform."""

    def __init__(self, w_in, w_out, stride, params):
        super(ResSortMaxBlurPoolBlock, self).__init__()
        self.proj, self.bn = None, None
        if (w_in != w_out) or (stride != 1):
            self.proj = conv2d(w_in, w_out, 1, stride=1)
            self.proj_blur = SortBlurPool(w_out, w_out, stride=stride)
            self.bn = norm2d(w_out)
        self.f = SortMaxBlurPoolTransform(w_in, w_out, stride, params)
        self.af = activation()
...
class AvgPoolTransform(Module):
    """Basic transformation: [3x3 conv, BN, Relu] x2."""

    def __init__(self, w_in, w_out, stride, _params):
        super(AvgPoolTransform, self).__init__()
        if stride != 1:
            self.a = conv2d(w_in, w_out, 3, stride=1)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
            self.max_blur = nn.AvgPool2d(kernel_size=stride, stride=stride)
        else:
            self.a = conv2d(w_in, w_out, 3, stride=stride)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ResAvgPoolBlock(Module):
    """Residual basic block: x + f(x), f = basic transform."""

    def __init__(self, w_in, w_out, stride, params):
        super(ResAvgPoolBlock, self).__init__()
        self.proj, self.bn = None, None
        if (w_in != w_out) or (stride != 1):
            self.proj = conv2d(w_in, w_out, 1, stride=stride)
            self.bn = norm2d(w_out)
        self.f = AvgPoolTransform(w_in, w_out, stride, params)
        self.af = activation()
...
class NormalCenterMaxBlurPoolTransform(Module):
    """Basic transformation: [3x3 conv, BN, Relu] x2."""

    def __init__(self, w_in, w_out, stride, _params):
        super(NormalCenterMaxBlurPoolTransform, self).__init__()
        if stride != 1:
            self.a = conv2d(w_in, w_out, 3, stride=1)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
            self.max_blur = NormalCenterBlurPool(w_out, w_out, stride=stride)
        else:
            self.a = conv2d(w_in, w_out, 3, stride=stride)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ResNormalCenterMaxBlurPoolBlock(Module):
    """Residual basic block: x + f(x), f = basic transform."""

    def __init__(self, w_in, w_out, stride, params):
        super(ResNormalCenterMaxBlurPoolBlock, self).__init__()
        self.proj, self.bn = None, None
        if (w_in != w_out) or (stride != 1):
            self.proj = conv2d(w_in, w_out, 1, stride=stride)
            self.bn = norm2d(w_out)
        self.f = NormalCenterMaxBlurPoolTransform(w_in, w_out, stride, params)
        self.af = activation()
...
class NormalSumMaxBlurPoolTransform(Module):
    """Basic transformation: [3x3 conv, BN, Relu] x2."""

    def __init__(self, w_in, w_out, stride, _params):
        super(NormalSumMaxBlurPoolTransform, self).__init__()
        if stride != 1:
            self.a = conv2d(w_in, w_out, 3, stride=1)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
            self.max_blur = NormalSumBlurPool(w_out, w_out, stride=stride, groups=w_out)
        else:
            self.a = conv2d(w_in, w_out, 3, stride=stride)
            self.a_bn = norm2d(w_out)
            self.a_af = activation()
...
class ResNormalSumMaxBlurPoolBlock(Module):
    """Residual basic block: x + f(x), f = basic transform."""

    def __init__(self, w_in, w_out, stride, params):
        super(ResNormalSumMaxBlurPoolBlock, self).__init__()
        self.proj, self.bn = None, None
        if (w_in != w_out) or (stride != 1):
            self.proj = conv2d(w_in, w_out, 1, stride=stride)
            self.bn = norm2d(w_out)
        self.f = NormalSumMaxBlurPoolTransform(w_in, w_out, stride, params)
        self.af = activation()
...
class ResMaxBlurPoolBottleneckBlock(Module):
    ...
    def __init__(self, w_in, w_out, stride, params):
        ...
        if (w_in != w_out) or (stride != 1):
            self.proj_blur = BlurPool(w_in, filt_size=3, stride=stride)
            # self.proj_blur = self.max_blur = BlurPool2(w_in, w_in, kernel_size=3, stride=stride, groups=w_in)
            self.proj = conv2d(w_in, w_out, 1, stride=1)
            self.bn = norm2d(w_out)
...
class MaxParamBlurPoolBottleneckTransform(Module):
    """Bottleneck transformation: 1x1, 3x3 [+SE], 1x1."""

    def __init__(self, w_in, w_out, stride, params):
        super(MaxParamBlurPoolBottleneckTransform, self).__init__()
        w_b = int(round(w_out * params["bot_mul"]))
        w_se = int(round(w_in * params["se_r"]))
        groups = w_b // params["group_w"]
        self.a = conv2d(w_in, w_b, 1)
...
class ResMaxParamBlurPoolBottleneckBlock(Module):
    """Residual bottleneck block: x + f(x), f = bottleneck transform."""

    def __init__(self, w_in, w_out, stride, params):
        super(ResMaxParamBlurPoolBottleneckBlock, self).__init__()
        self.proj, self.bn = None, None
        if (w_in != w_out) or (stride != 1):
            self.proj_blur = ParamBlurPool3x3(w_in, w_in, kernel_size=3, stride=stride, groups=w_in)
            self.proj = conv2d(w_in, w_out, 1, stride=1)
            self.bn = norm2d(w_out)
        self.f = MaxParamBlurPoolBottleneckTransform(w_in, w_out, stride, params)
        self.af = activation()
