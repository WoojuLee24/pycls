...
import numpy as np
import torch
from pycls.datasets.augmentations import *
...
def aug(image, preprocess):
    ...
    ws = np.float32(
        np.random.dirichlet([aug_prob_coeff] * aug_mixture_width))
    m = np.float32(np.random.beta(aug_prob_coeff, aug_prob_coeff))

    mix = torch.zeros_like(preprocess(image))
    for i in range(aug_mixture_width):
        image_aug = image.copy()
        depth = aug_mixture_depth if aug_mixture_depth > 0 else np.random.randint(1, 4)
        for _ in range(depth):
            op = np.random.choice(aug_list)
            image_aug = op(image_aug, aug_severity)
        # Preprocessing commutes since all coefficients are convex
        #mix += ws[i] * preprocess(image_aug)
        k = ws[i] * preprocess(image_aug)
        mix = mix + k
